function nia_sbo_displayCombiPlots(fig, data, x_label, y_label, colors, live_info)
%NIA_DISPLAYCOMBIPLOTS Show traces combined in a single plot
...
for idx=1:length(data)
    ...
    hold(udata.axes_list{1}, 'on');
    
    udata.plots_list{idx} = plot(...
        udata.axes_list{1}, dset(1,:), dset(2,:));
        
    set(udata.plots_list{idx}, ...
        'LineWidth', 1.5, ...
        'Color', clr);
    
    ax = udata.axes_list{1};

    xlabel(ax, x_label);  
    
    xlim(ax, [0 size(dset,2)]); % Set tighter x-axis limits
%     ylim(ax, [0 1.2*max(dset(2,:))] ); % and looser y-axis limits
    
    ylabel(ax, y_label);   
    
    set(ax, 'Layer', 'Top'); % Bring axes in front of plot lines
end

% Link the axes
% linkaxes([udata.axes_list{:}], 'x');

% Switch off axes hold
hold(udata.axes_list{1}, 'off');
...
% Toggle bind 'on' by default
set(bind_ctrl, 'State', 'on');
bindOnCallback(fig);
...
% MODIFIED FOR SINGLE COMBINED PLOT:
for roi_idx=1:length(udata.live_info.uuids)
    creator_udata.addListenerToROI(udata.live_info.uuids{roi_idx}, ...
        udata.plots_list{roi_idx});
end
