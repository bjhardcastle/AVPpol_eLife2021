...
params.addParameter('interval','tinterval', @(x) ischar(x) && any(strncmpi(x,{'sem','tinterval'},4)) );
...
function [h,statsOut]=myPlotter(X,Y)
    ...
    for k=1:length(X)
        ...
        statsOut(k).sd = SD(k);
        statsOut(k).N = length(thisY);
        statsOut(k).vals = thisY;
    ...
    function X=violaPoints(X,Y)
        ...
        for jj=1:length(inds)
            ...
            xpoints = linspace(-tWidth, tWidth, counts(jj));
...
% original with only my bug fix for stats output 
%{
function varargout=notBoxPlot(y,x,varargin)
% notBoxPlot - Doesn't plot box plots!
%
