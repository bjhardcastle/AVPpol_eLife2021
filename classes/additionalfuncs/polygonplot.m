...
% ----------------------------------------------------------------------- %
function polygonplot(data,opt_axes,opt_lines,opt_area)
    ...
    if(~isfield(opt_lines,'Labels')) ||existingAxes   % Text labels
        labels = false;
    else
    ...
    if(~isfield(opt_lines,'Legend'))
     ...
     if(isfield(opt_axes,'NRadii'))
        if(length(opt_axes.NRadii)~=1) || ~isnumeric(opt_axes.NRadii)
            error('NRadii must be a single value.');
        end
     end
%      if(isfield(opt_axes,'Labels'))
%         if(length(opt_axes.Labels)~=N) && ~(isfield(opt_axes,'NRadii'))
%             error('You must provide N axis labels.');
%         elseif (length(opt_axes.Labels)~=N)
%             error('You must provide N axis labels.');
%         end
%     end
    set(gcf,'Color',[1 1 1]);
    ...
    data_min = min(data(:));
    data_max = max(  max(mean(data,3)) + 0.5*(mean(std(data,[],3)/sqrt(size(data,3))))  );
    
    if(~isfield(opt_axes,'Ticks'))
        opt_axes.Ticks = linspace(data_min,data_max,9);
%     elseif(length(opt_axes.Ticks)<2)
%         error('There should be more than 2 isocurves.');
    end
    if ~isempty(opt_axes.Ticks)
    ...
    h_iso = line([x,x(:,1)]',[y,y(:,1)]','LineWidth',0.5,'Color',0.9.*ones(1,3));
    for iso_id = 1:1:length(h_iso)   % Exclude axes from legend
        set(get(get(h_iso(iso_id),'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
        uistack(h_iso(iso_id),'bottom')
    end
    hold on;
    end
    ...
    if ~existingAxes
        if(isfield(opt_axes,'NRadii'))
            NRadii = opt_axes.NRadii;
        else
            NRadii = N;
        end
    
    th_jump = (2*pi/NRadii)*(ones(2,1)*(NRadii:-1:1));
    radii   = [zeros(1,NRadii); ones(1,NRadii)];
    [x,y]   = pol2cart(th_jump, radii);
    h_axes  = line(x,y,'LineWidth',0.5,'Color',0.9.*ones(1,3));
    for ax_id = 1:1:NRadii   % Exclude axes from legend
...
%         text(loc(t),odd_v(t),num2str(opt_axes.Ticks(t)),'Background',opt_axes.Background,'FontSize',7);
%     end
    
    % Display axis labels
    if(isfield(opt_axes,'Labels')) && ~existingAxes
          if(isfield(opt_axes,'NRadii'))
            NRadii = opt_axes.NRadii;
        else
            NRadii = N;
        ...
        th_lbl = (2*pi/NRadii)*(NRadii:-1:1);
        r_lbl  = 1.1.*ones(1,NRadii);
        [xlbl,ylbl] = pol2cart(th_lbl,r_lbl);
        for lid = 1:1:NRadii
            text(xlbl(lid),ylbl(lid),opt_axes.Labels{lid},'HorizontalAlignment','center');
        end
    end
    ...
    if(M == 0)    % Only mean
        ...
        switch(opt_area.err)
            ...
            case 'sem', err = (data_std./sqrt(size(data,3)));
            case 'var', err = (data_std.^2);
            case 'c95', err = (data_std./sqrt(size(data,3))).*1.96;
        end
