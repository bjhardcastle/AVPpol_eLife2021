function fig = nia_sbo_playFlatMovie(obj, mov, acceptROIs)
...
%   corresponds to channel. The argument acceptROIs may be omitted, or it
%   may contain a function handle that should be invoked prior to closing
%   the window. This function returns the figure handle for the main
%   window.
%
%   Example:
...
callback_list.saveROIs = @saveROIs;

fig = nia_sbo_playGenericMovie(obj, mov, callback_list);
end
...
for ch_idx=1:num_ch
    ...
    %}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Trying this alternative code for the above
    
    if numel(mov(:,:,:,ch_idx)) > cfg_max_sample
        sample_indices = randperm(cfg_max_sample);
    else
        sample_indices = 1:numel(mov(:,:,:,ch_idx));
    end
    m = mov(mov~=0);
...
%     vec(2,:) = nia_sbo_scanROIFlatMovie(...
%         mov, mask, roi_list(idx).channel);
%     
    vec(2,:) = sbo_scanROI(mov,mask);
...
function saveROIs(obj, roi_list)
% This function saves the passed ROIs to the object
