function ca_RegSeries_v4x2_InputArgs(im0, path0, poolSiz, walkThresh, usfac)
...
warning(w);
% Loop to repeat registration twice
for iIdx = 1:2
poolSiz = poolArray(iIdx);
walkThresh = walkArray(iIdx);
usfac = usfacArray(iIdx);

% If PoolSiz is larger than number of frames in tif, reduce it to match
if poolSiz > size(inStk,3)
    poolSiz = size(inStk,3);
end
...
sint_SaveStack_v2(inStk,16,path0,im1,im0);
