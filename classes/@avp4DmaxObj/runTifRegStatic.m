function runTifRegStatic(obj, poolSiz, walkThresh, usfac)
...
if nargin < 3
    if isprop(obj,'walkThresh') && ~isempty( obj.walkThresh )
        ...
        walkThresh = 500; % hard threshold to identify big jerks that need extra attention
    end
end
if nargin < 2
...
% Run registration:
ca_RegSeries_v4_InputArgs_proportional(obj.TiffFile, local_dir, poolSiz, walkThresh, usfac);
...
if NETWORK_FILE
    ...
    copyfile([local_dir 'regDispXY.txt'], obj.Folder)
    rmdir(local_root, 's')
end