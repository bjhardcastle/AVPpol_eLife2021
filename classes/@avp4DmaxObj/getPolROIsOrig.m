function varargout = getPolROIsOrig(obj, numROIs, manualROIs)
if nargin< 3 || isempty(manualROIs)
    manualROIs = 0;
end
...
if nargin< 2|| isempty(numROIs)
    numROIs = 1;     % maximum number of areas to be combined, per angle presented
elseif numROIs > 16
    numROIs = 16;
...
polAngImg = reshape(maxIdx,size(f)).*mode(diff(polAngs));
polAngImg = polAngImg.*(polSelImg>obj.polSelThreshold);
...
% polAngImg = reshape(maxIdxNarrow,size(f)).*mode(diff(polAngs));
% polAngImg = polAngImg.*(polSelImg>obj.polSelThreshold);
...
%}
% connMask = smoothPolAngImg(polAngImg);


polAngImg(polAngImg == 0) = nan;
k = polAngImg./mode(diff(polAngs));
...
respOrientImg(isnan(polAngImg)) = nan;
...
for l = 1:length(u)
    ...
    fullROI(ridx).mask = k==u(l);
    fullROI(ridx).color = polColMap(u(l),:);
    fullROI(ridx).position = [1 1 1 1];
    fullROI(ridx).response = scanROI(obj,[k==u(l)]);
    if u(l) ~= controlROIidx
        fullROI(ridx).angle = polAngs(u(l));
    else
        fullROI(ridx).angle = nan;
    end
end
...
for n = 1:length(uniqueAng)
    ...
    for rIdx = 1:length(rp)
        ...
        elseif any( mode(perimPixVals(perimPixVals>0)) == plusminus1Ang(n,:))
            ...
            outerPix = find(grayconnected(polAngImgCopy,arow(1),acol(1)));
