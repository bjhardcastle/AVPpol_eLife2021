...
for tidx = 1:length(obj.TrialStartFrame)
    ...
    if size(obj.Daq,2) >= 6
        ...
        ch6(tidx) = mean(obj.Daq( obj.TrialStartSample(tidx) : obj.TrialEndSample(tidx) ,6) );
    end
...
% Ch6 and Ch7 data
obj.TrialCh6 = round(ch6);
obj.TrialCh7 = round(ch7);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Mod for avp experiments:
% Wherever ch3 == 2 | 3 | 4  the voltage on ch2 corresponds to Pol Angle 
% all angles are sent/recorded as:
%  +ve increase = ccw rotation, as looking at the polarizer, into the LED
obj.TrialPatNum( obj.TrialSeqNum == 2 ) = round(ch2( obj.TrialSeqNum == 2 )*360/5);
obj.TrialPatNum( obj.TrialSeqNum == 3 ) = round(ch2( obj.TrialSeqNum == 3 )*360/5);
obj.TrialPatNum( obj.TrialSeqNum == 4 ) = round(ch2( obj.TrialSeqNum == 4 )*360/5);
obj.TrialPatNum( obj.TrialSeqNum == 8 ) = round(ch2( obj.TrialSeqNum == 8 )*360/5);
obj.TrialPatNum( obj.TrialSeqNum == 10 ) = round(ch2( obj.TrialSeqNum == 10 )*360/5);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Mod for avp experiments:
% If the experiment was run in reverse, angles were recorded in opposite
% direction:
%  +ve increase = cw rotation, as looking at the polarizer, into the LED
% convert to +ve increase = ccw rotation:
for polExpNum = [2,4,8,10]
    if length(obj.pSet)>= polExpNum % not all experiments include pSet(10)
        if isfield(obj.pSet(polExpNum),'StepDIR') && obj.pSet(polExpNum).StepDIR == 5
            obj.TrialPatNum( obj.TrialSeqNum == polExpNum ) = 360 - obj.TrialPatNum( obj.TrialSeqNum == polExpNum );
            obj.TrialPatNum( obj.TrialSeqNum == polExpNum & obj.TrialPatNum == 0 ) = 360;
        end
    end
end
...
%  disp('NOT Detecting stimulus on/off times..')
