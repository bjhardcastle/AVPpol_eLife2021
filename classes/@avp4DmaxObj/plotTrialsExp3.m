function varargout = plotTrialsExp3(objarray, ROImaskidx, fields, errorbar, plotcolor, tfig)
...
[responseArray, timeVector, F0Array, respIdx] = findRespArrayExp3(objarray, ROImaskidx, fields);
...
if nargout == 1 
    ...
    outputData = struct;
    if  ~isempty(meanTrials)
    outputData.maxOfMean = max(meanTrials,[],2);
    outputData.maxOfFlies = max(fly_mean_traces,[],2);
    outputData.minOfMean = min(meanTrials,[],2);
    outputData.minOfFlies = min(fly_mean_traces,[],2);
    outputData.meanTrial = meanTrials;
    outputData.populationSize = populationSize;
    outputData.flyMeanTraces = fly_mean_traces;
    outputData.responseArray = responseArray;
    outputData.timeVector = timeVector;
    outputData.F0Array = F0Array;
    outputData.respIdx = respIdx;
    outputData.fig = tfig;
    outputData.ax = taxes;
    end
