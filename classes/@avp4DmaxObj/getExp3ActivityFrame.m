function varargout = getExp3ActivityFrame(obj)
...
angles = obj.expPolAngs(3);
exp3start = obj.expStart(3);
exp3stop = obj.expStop(3);

avgTrialLength = median(diff([obj.TrialStartFrame(obj.TrialSeqNum == 3); obj.TrialEndFrame(obj.TrialSeqNum == 3)]));
...
for aIdx = 1:length(angles)
    ...
    imagesc(aFrame{aIdx})
    polarmap
    title([num2str(angles(aIdx)) '\circ'])
    axis image
    grid on
    set(gca,'XTickLabel',{});
    set(gca,'YTickLabel',{});
