function varargout = getExp7ActivityFrame(obj)
...
if any(obj.TrialSeqNum == 7)
    angles = unique(obj.TrialPatNum(obj.TrialSeqNum == 7));
    expNum = 7;
elseif any(obj.TrialSeqNum == 9)
    expNum = 9;
    angles = 1:3; % middle set of three is recorded as 'PatNum' 3, same as first set, so we have to manually select for the middle three
     fields=[];
    fields.TrialSeqNum = expNum;
    exp9TrialIdx = findTrials(obj,fields)';

else
    disp('Exp 7/9 not recorded');
    return
end
...
for aIdx = 1:length(angles)
    ...
    elseif expNum == 9
           PrefTrialIdx = exp9TrialIdx((aIdx-1)*3 + 1 : (aIdx-1)*3 +3 );
    end
