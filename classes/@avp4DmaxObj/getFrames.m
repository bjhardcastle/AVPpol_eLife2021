...
if isempty(obj.UseMSP) || ~obj.UseMSP
...
else % load MSP frames .mat
    filename = [obj.DateStr obj.TimeStr '_MSP.mat'];
    folder = [obj.Folder];
    if exist(fullfile(folder,filename),'file')
        load( fullfile(folder,filename), 'Frames' );
        if scan_extent(1) == 1 && scan_extent(end) == 0
            stkStruct = Frames;
        else
            stkStruct = Frames(:,:,scan_extent(1):scan_extent(end));
        end
    else
        disp(['MSP has not been saved: ' fullfile(folder,filename)])
    end
end
...
if nargin < 2
    ...
    if isempty(obj.UseMSP) || ~obj.UseMSP
        ...
        obj.ActivityFrame = max(stkStruct,[],3);
                obj.ActivityFrame1 = std(stkStruct,[],3);
    end
    
    % If the background had previously been subtracted from the stored
