function [responseArray, timeVector, F0Array, ObjIdx,stimTimeArray] = findRespArrayExp6(objarray, ROImaskidx, fields)
...
for oidx = 1:length(objarray)
    barExp = 6;
     numBarTrials = 8;
if  ismember(barExp,objarray(oidx).TrialSeqNum)  %  bar mapping exists
        
        inclObjIdx(end+1)= oidx;
        s0(oidx) =  objarray(oidx).TrialStartSample(find([objarray(oidx).TrialSeqNum == barExp],1,'first'));
        s1(oidx) =  objarray(oidx).TrialEndSample(find([objarray(oidx).TrialSeqNum == barExp],1,'first')+numBarTrials-1);
        
        t0(oidx) =  objarray(oidx).TrialStartFrame(find([objarray(oidx).TrialSeqNum == barExp],1,'first'));
        t1(oidx) = objarray(oidx).TrialEndFrame(find([objarray(oidx).TrialSeqNum == barExp],1,'first')+numBarTrials-1);
        
            sStartSidx(oidx,:) = objarray(oidx).TrialStartSample(find([objarray(oidx).TrialSeqNum == barExp],1,'first'):find([objarray(oidx).TrialSeqNum == barExp],1,'first')+numBarTrials-1);      
        sEndSidx(oidx,:)= objarray(oidx).TrialEndSample(find([objarray(oidx).TrialSeqNum == barExp],1,'first'):find([objarray(oidx).TrialSeqNum == barExp],1,'first')+numBarTrials-1);

        if ~isempty(s0)
            s(end+1) = ( s1(oidx) -s0(oidx) )/10000;
            t(end+1) = t1(oidx) -t0(oidx) ;        
            tStartFidx(end+1,:) = objarray(oidx).TrialStartFrame(find([objarray(oidx).TrialSeqNum == barExp],1,'first'):find([objarray(oidx).TrialSeqNum == barExp],1,'first')+numBarTrials-1);      
        tEndFidx(end+1,:)= objarray(oidx).TrialEndFrame(find([objarray(oidx).TrialSeqNum == barExp],1,'first'):find([objarray(oidx).TrialSeqNum == barExp],1,'first')+numBarTrials-1);
...
for oidx = 1:length(objarray)
    ...
    F0_returned( tcount, : ) = findF0Exp5( objarray(oidx), mask, trials_returned( tcount , : ),[], timeVector, normFPS  );
