...
X12 = imfuse(X2,X35,'blend','Scaling','joint');
...
frame = imcomplement(invFactor*X12);
...
f.Position(4) = 6*size(frame,2);
    ...
    print(f, fullfile(obj.Folder,['polColor_w_90-180',cropName{n}]) , '-dpng','-r450');
close(f)
...
f.Position(4) = 6*size(frame,2);
    set(f, 'InvertHardCopy', 'off');
    print(f, fullfile(obj.Folder,['polColor_b_90-180',cropName{n}]) , '-dpng','-r450');
 close(f)
