function getPolMapsHalfCycle(obj)
...
% For analysis of half cycles:

    obj.TrialPatNum(obj.TrialPatNum>180) =  obj.TrialPatNum(obj.TrialPatNum>180) - 180;
    polAngs(polAngs>180) = polAngs(polAngs>180) - 180;

polAngs(polAngs == 360) = 180;
obj.TrialPatNum(obj.TrialPatNum == 360) = 180;
polAngs(polAngs == 0) = 180;
obj.TrialPatNum(obj.TrialPatNum == 0) = 180;
polAngs = unique(polAngs);
...
sortedPolAngs = sort(polAngs);
angEdges = [sortedPolAngs-0.5*(mode(diff(sortedPolAngs))) sortedPolAngs(end)+0.5*(mode(diff(sortedPolAngs)))]  ;

% deal with 0/180 and make bins centered on each angle:

shiftAng =0.5.*(wrapTo360((2.*(rad2deg(theta)-0.5*mode(diff(polAngs))))))  + min(angEdges);

maxIdx = discretize(shiftAng,angEdges);  

% For each orientation get the non-pooled response ang+ang+180):
    pixPolResp = [];
    for aIdx = 1:length(polAngs)
        % Get logic array that indicates every frame that was recorded at the
        % angle (without pooling angle+180 frames)
        angleFrameArray = getAngleLogicArray2(obj,polAngs(aIdx));
...
respMin = pixPolResp( sub2ind(size(pixPolResp), [1:indLength]', 1+mod(maxIdx + 0.5*length(polAngs),length(polAngs)) ) );
maxIdx = maxIdx + 1;
...
% between 0:1
polSelVec(polSelVec==1) = 0; % usually from image registration
