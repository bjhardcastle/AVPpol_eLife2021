function makeColorStackVid(objarray)
...
for oidx = 1:length(objarray)
    ...
    x1 = getPolAngleActivityFrame(g,30);
    x15 = getPolAngleActivityFrame(g,60);
    x2 = getPolAngleActivityFrame(g,90);
    x25 = getPolAngleActivityFrame(g,120);
    x3 = getPolAngleActivityFrame(g,150);
    x35 = getPolAngleActivityFrame(g,180);
    
    
    x1scaled = x1.*(x1>0)./max(x1(:));
    X1 = cat(3,zeros(size(x1)),x1scaled,zeros(size(x1)));
    X1inv = imcomplement(X1);
    
    x15scaled = x15.*(x15>0)./max(x15(:));
    X15 = cat(3,zeros(size(x15)),x15scaled,zeros(size(x15))); % Green > Purple
    X15inv = imcomplement(X15);
    
    x2scaled = x2.*(x2>0)./max(x2(:));
    X2 = cat(3,x2scaled,zeros(size(x2)),x2scaled); % Purple > Green
    X2inv = imcomplement(X2);
    
    x25scaled = x25.*(x25>0)./max(x25(:));
    X25 = cat(3,x25scaled,zeros(size(x25)),x25scaled); % Purple > Green
    X25inv = imcomplement(X25);
    
    
    x3scaled = x3.*(x3>0)./max(x3(:));
    X3 = cat(3,x3scaled,x3scaled,zeros(size(x3))); % Yellow > blue
    X3inv = imcomplement(X3);
    
    x35scaled = x35.*(x35>0)./max(x35(:));
    X35 = cat(3,x35scaled,x35scaled,zeros(size(x35))); % Yellow > BLue
    X35inv = imcomplement(X35);
    
    X12inv = imfuse(X1inv,X2inv,'blend','Scaling','independent');
    X123inv = imfuse(X12inv,X3inv,'blend','Scaling','independent');
    
    X12binv = imfuse(X15inv,X25inv,'blend','Scaling','independent');
    X123binv = imfuse(X12binv,X35inv,'blend','Scaling','independent');
    
    X123abinv = imfuse(X123inv,X123binv,'blend','Scaling','independent');
    
    
    angleStk(:,:,objarray(oidx).ZPlane,1) = X123inv;
