function snapStruct = superCyclePolarHalfResp(objarray,roiIdx)
...
for oidx = 1:length(objarray)
    ...
    if ( ~isempty(objarray(oidx).MIP.polExp) && objarray(oidx).MIP.polExp == 4 ) ... % only look at regular mapping exps (so tuning shifts are compared fairly across exps)
        ...
        if isempty(objarray(oidx).MIP.stdPolImg)
        ...
        elseif useFirstFourCyclesOnly
            ...
            numTrialsOrig = objarray(oidx).MIP.pSet(4).trialReps*2*trialsPerCycle;
            ...
            for caseIdx = 1
                ...
                for cycIdx = 1:size(trialArray,1) % number of cycles
                    ...
                    objarray(oidx).MIP.UseFixedResp = 0;
                     for rIdx = roiIdx
                         [ROImeanTuningAng,ROIresultantLength] = tuningROI(objarray(oidx).MIP,objarray(oidx).MIP.ROI(rIdx).mask,1);
                    
                    % store angle tunings
                    snapStruct(oidx).(caseStr{caseIdx}).ROImeanTuningAng(cycIdx,rIdx) = ROImeanTuningAng;
                    snapStruct(oidx).(caseStr{caseIdx}).ROIresultantLength(cycIdx,rIdx) = ROIresultantLength;
