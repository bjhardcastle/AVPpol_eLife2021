function plotPolMapTrialManual(objarray,ROIidx,errorbar,numCycles)
        ...
        for oidx = 1:length(objarray)            
            ...
            elseif uselayermask
                ...
                if ~isempty(objarray(oidx).MIP.layerMask)
                    ...
                    objarray(oidx).MIP.ROI(1).response = scanROI(objarray(oidx).MIP,objarray(oidx).MIP.layerMask.mask);
...
%             end
        ...
        if ~isempty(inclObjIdx)
            [~,~,stimTimes]=plotTrialsExp4([objarray([inclObjIdx]).MIP],roiAngs(roiIdx),[],errorbar,[],f);
