function superPBXY(obj,Yoff,Xoff,rotateOFF, darkMode)
...
% PB version finds pol maps without pooling responses to ang+180 and plots
% correlation over 360deg
...
for oidx = 1:length(obj)
    ...
    if ~any(obj(oidx).Layers(1).TrialSeqNum==4) && ~any(obj(oidx).Layers(1).TrialSeqNum==2) && ~any(obj(oidx).Layers(1).TrialSeqNum==8)
        disp('No pol exp (2,4,8) available')
        return
    end
    ...
    getPolMapsUnpooled(obj(oidx).MIP)
...
elseif strcmp(obj(oidx).Area,'PB')
        ...
        bounds = [-2 2];
posX2 = 2*posX;
    [rho, pval, phi_0, amax] = circlin_wcorr(deg2rad(angD), mod(posX2,1),bounds, W);
    phi_fit = 360*(1*amax)*posX*2 + rad2deg(phi_0);
     if runPermutationTest
    ...
    for phi_shift = [-1080:360:1080]
        
        this_phi_fit = phi_fit + phi_shift;
            L = plot(posX(this_phi_fit>=0 & this_phi_fit<=360),this_phi_fit(this_phi_fit>=0 & this_phi_fit<=360),colstr);
