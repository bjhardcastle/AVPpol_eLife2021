function varargout = superPolAngHist(objarray,useTuningColors,usePSIthreshold)
...
for oidx = 1:length(objarray)
    if objarray(oidx).containsPolMapExp>=4 ...
        ...
        if sum(~isnan(maskedPolTuningFrameVector)) > 0.1*sum(objarray(oidx).MIP.layerMask.mask) % don't include if only a few pixels respond
            shiftAng = 0.5.*(wrapTo360((2.*( maskedPolTuningFrameVector - shiftedBinEdges(1)))))  + shiftedBinEdges(1);
...
switch vers
    case 'mono'
        % regular version
        histHandles = polarhistogram('BinEdges',deg2rad(plotBins),'BinCounts',([plotTrials'; plotTrials']));
    ...
    case 'color'
        ...
        for pidx = 1:2*length(meanTrials)
            histHandles(pidx) = polarhistogram('BinEdges',[deg2rad([plotBins(pidx),plotBins(pidx+1)])],'BinCounts',(plotTrials(mod(pidx-1,length(meanTrials))+1)));
...
ax.RLim = [0  max(plotTrials)+0.05];
if max(plotTrials) > max(ax.RLim)
