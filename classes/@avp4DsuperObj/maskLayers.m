...
% end
%
loadLayerMasks([superobj.Layers, superobj.MIP])
newLayer = 1;
currLayer = 0;
currROI = [];
MIPROI = [];
...
% Save callbacks (write to disk):
    function save_ctrl_cb(varargin)
        pushMIP2Obj
        pushCurr2Obj
        saveLayerMasks([superobj.Layers,superobj.MIP])
        updateAll
        disp('Saved.')
    end
...
function xycorrMIP_ctrl_cb(varargin)
    ...
    function save_layer_cb(varargin)
        pushCurr2Obj
        saveLayerMasks(superobj.Layers(currLayer))
        updateAll
    end
    function save_MIP_cb(varargin)
        pushMIP2Obj
        saveLayerMasks(superobj.MIP)
        updateAll
    end
...
% Reload callbacks (load from disk and apply ROI)
    function reload_layer_cb(varargin)
        delete(currROI);
        currROI = [];
        loadLayerMasks(superobj.Layers(currLayer))
        updateAll
    end
    ...
    function clear_ALL_cb(varargin)
        ...
        delete(currROI)
        currROI=[];
        saveLayerMasks([superobj.Layers])
        updateAll
        disp('Saved.')
...
%                             getActivityFrameMax(superobj.Layers(currLayer));
    ...
    function updateMask(varargin)
        ...
        elseif ~isempty(superobj.MIP.layerMask)
            delete(MIPROI);
            MIPROI = impoly(MIP_ax,superobj.MIP.layerMask.position);
            MIPROI.setColor([1 1 1]);
            fcn = makeConstrainToRectFcn('impoly', ...
