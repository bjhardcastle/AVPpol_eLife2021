function superPolAngResultant(objarray,usePSIweighting,usePSIthreshold)
...
for oidx = 1:length(objarray)
    if objarray(oidx).containsPolMapExp>=4 ...
        ...
        if sum(~isnan(maskedPolTuningFrameVector)) > 0.1*sum(objarray(oidx).MIP.layerMask.mask) % don't include if only a few pixels respond
            ...
            [r,axtheta] = circ_rangle(circ_axial(deg2rad(maskedPolTuningFrameVector),2), weightingVector, [], 1);
            theta = axtheta/2;
