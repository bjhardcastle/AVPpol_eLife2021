function plotPolMapTrial(objarray,roiAngs,errorbar,numCycles)
...
for angIdx = 1:length(plotangs)    
    ...
    for roiIdx = 1:length(roiAngs)
        ...
        for oidx = 1:length(objarray)            
            ...
            if ( isfield(objarray(oidx).MIP.pSet(4),'polOffBetweenTrials') && objarray(oidx).MIP.pSet(4).polOffBetweenTrials ) ...
                || ( isfield(objarray(oidx).MIP.pSet(4),'trialRandomizeOrder') && objarray(oidx).MIP.pSet(4).trialRandomizeOrder )
                continue
            end
