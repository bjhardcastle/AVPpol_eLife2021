function plotBarMapTrial(objarray,roiAngs,errorbar,numCycles)
manualROIs = 0;
    ...
    for roiIdx = 1:length(roiAngs)
        ...
        for oidx = 1:length(objarray)
            ...
            if ~isempty(inclObjIdx)
                if ~manualROIs
                    [~,~,stimTimes]=plotTrialsExp5([objarray([inclObjIdx]).MIP],1,[],errorbar,[],f);
                else
                    [~,~,stimTimes]=plotTrialsExp5([objarray([inclObjIdx]).MIP],ROIang,[],errorbar,[],f);
                end
...
%         addAngleIndicators(ax(1),xstim(1,:))
        if ~isequal(numCycles,[1,4])
            ax(1).XLim(1) =  xstim(1,floor(numCycles(1)*size(xstim,1)-1));
            
            ax(1).XLim(2) =  xstim(1,floor(numCycles(2)*size(xstim,1)));
        end
    ...
    function addStimShading(ax,xstim)
        clims = [0.85,0.95];
        cols = linspace(clims(2),clims(1),5)'; % grey cols for 5 angles
