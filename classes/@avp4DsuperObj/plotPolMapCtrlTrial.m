function plotPolMapCtrlTrial(objarray,roiAngs,errorbar,numCycles)
...
for angIdx = 1:length(plotangs)    
    ...
    for roiIdx = 1:length(roiAngs)
        ...
        for oidx = 1:length(objarray)            
            ...
            if ( isfield(objarray(oidx).MIP.pSet(2),'polOffBetweenTrials') && objarray(oidx).MIP.pSet(2).polOffBetweenTrials ) ...
                || ( isfield(objarray(oidx).MIP.pSet(2),'trialRandomizeOrder') && objarray(oidx).MIP.pSet(2).trialRandomizeOrder )
                continue
            end
        ...
        if ~isempty(inclObjIdx)
            [~,~,stimTimes]=plotTrialsExp4([objarray([inclObjIdx]).MIP],1,[],errorbar,[],f);
