...
%         p = plotPSIpatch(axH,xcenters(x),ycenters(y),psi(y,x));
                p = plotPSIpatch(axH,xcenters(x),ycenters(y),tuning(y,x));
%                 p = plotPSIpatch(axH,xcenters(x),ycenters(y),(90+wrapTo360(-tuning(y,x)-270))/180);
%                 p = plotPSIpatch(axH,xcenters(x),ycenters(y),tuning(y,x)/180);
...
function p = plotPSIpatch(ax,xc,yc,psi)
% in this version the patch colormap codes tuning angle [range 0:180], not psi 
% colors = [1 1 1;flipud(magma(200))];
% colors = [1 1 1;flipud(gray(200))];
% psi = abs(psi-90)/125; 
%  [colors] = colMapGen([1 0 1],[0 1 1],201);
  [colors] = colMapGen([239,138,98]./255,[103,169,207]./255,201);
%   [colors] = colMapGen([231,58,24]./255,[23,156,232]./255,201);

% colors = [flipud(coolwarm(201))];
psi = 0.5+((psi-90)/180);
% psi = 0.5+((psi-90)/270); % here dividing by 180 would use the full range of the colormap
psicol = colors(round(200*psi+1),:); 
x = [xc-0.5  xc+0.5 xc+0.5 xc-0.5];
y = [yc-0.5 yc-0.5 yc+0.5 yc+0.5];
p = patch(ax,x,y,psicol,'EdgeColor','none');
uistack(p,'bottom');
